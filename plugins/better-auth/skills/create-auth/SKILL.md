...
Before relying on examples, read the bundled [Better Auth LLM index](../../llms.txt)
and use the official documentation matching the installed `better-auth` version.
The versioned documentation overrides this vendored guidance when they differ.
...
- **Package manager** — Use Bun and check for `bun.lock` or `bun.lockb`.
...
### Steps
1. Install `better-auth` (the CLI ships as the `auth` package and is run via `bunx auth@latest`, no install needed)
...
| Adapter | Command |
|---------|---------|
| Built-in Kysely | `bunx auth@latest migrate` (applies directly) |
| Prisma | `bunx auth@latest generate --output prisma/schema.prisma` then `bunx prisma migrate dev` |
| Drizzle (dev) | `bunx auth@latest generate --output src/db/auth-schema.ts` then `bunx drizzle-kit push` |
| Drizzle (prod) | `bunx auth@latest generate --output src/db/auth-schema.ts` then `bunx drizzle-kit generate` then `bunx drizzle-kit migrate` |
